...
assay( rlog( deseqObj, blind=FALSE ) ) %>%
  ...
  write_csv( x = ., path =  here::here( 'results', 'gene_all_sample_rlog.csv.gz' ) ) 
```
...
assay( rlog( deseqObj, blind=FALSE ) ) %>%
  as.data.frame( . ) %>%
  rownames_to_column( "gene_id" ) %>%
  	write_csv( x = ., path =  here::here( 'results', 'unspliced_all_sample_rlog.csv.gz' ) ) 
```
