...
```{r correlated_ids}
# DLCO
dlco_id <- read_tsv( file = here( 'results', 'wgcna', 'PBMC_unspliced_WGCNA_table_DLCO_perc_pred.tsv' ) ) %>%
  filter( p.GS.Status < 0.05 ) %>%
  select( gene_id ) %>%
  mutate( TmpTrait = "DLCO" )

# Forearm MRSS
forearm_id <- read_tsv( file = here( 'results', 'wgcna', 'PBMC_unspliced_WGCNA_table_Forearm_MRSS.tsv' ) ) %>%
  filter( p.GS.Status < 0.05 ) %>%
  select( gene_id ) %>%
  mutate( TmpTrait = "Forearm MRSS" )

# Total MRSS
totalmrss_id <- read_tsv( file = here( 'results', 'wgcna', 'PBMC_unspliced_WGCNA_table_MRSS.tsv' ) ) %>%
  filter( p.GS.Status < 0.05 ) %>%
  select( gene_id ) %>%
  mutate( TmpTrait = "Total MRSS" )

# Merged
merged_ids <- rbind( dlco_id, forearm_id ) %>%
  rbind( ., totalmrss_id ) %>%
